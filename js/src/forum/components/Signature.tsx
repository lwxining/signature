  ...
  renderSignature() {
    return (
      <div className="Signature-content" onclick={this.edit.bind(this)}>
        {!this.user.signature() ? <p><i class="fa-solid fa-signature" style="color: #74C0FC;"></i>{app.translator.trans('signature.forum.profile.click')}</p> : m.trust(this.user.signatureHtml())}
      </div>
    );
  }
